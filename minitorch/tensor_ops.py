...
class SimpleOps(TensorOps):
    ...
    def reduce(
        ...
        Args:
            fn: function from two floats-to-float to apply
            a (:class:`TensorData`): tensor to redue over
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        for i in range(len(out)):
            # get the index of i in out_shape, store in out_index
            to_index(i, out_shape, out_index)
            # get in_index
            broadcast_index(out_index, out_shape, in_shape, in_index)
            # convert the index to position for both in and out
            out_position = index_to_position(out_index, out_strides)
            in_position = index_to_position(in_index, in_strides)
            # apply function for input
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for i in range(len(out)):
            ...
            # get out_position
            out_position = index_to_position(out_index, out_strides)
            # get a_index, then convert to get position
            broadcast_index(out_index, out_shape, a_shape, a_index)
            a_position = index_to_position(a_index, a_strides)
            # get b_index, then convert to get position
