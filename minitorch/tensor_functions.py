...
class ReLU(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Any:
        (t1,) = ctx.saved_values
        # return (minitorch.Tensor(t1.f.relu_back_zip(t1, grad_output),backend=grad_output.backend),0.0)
...
class Permute(Function):
    @staticmethod
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Any:
        # order=[int(order[x]) for x in range(order.size)]
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Any:
        ...
        # to find the index of order as the new order
