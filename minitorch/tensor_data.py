...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    # make it iterate from the last dimension to the first, because col first and row next
    cur_ord = ordinal + 0
...
def broadcast_index(
    ...
    for i, s in enumerate(shape):
        if s > 1:
            ...
            out_index[i] = 0
            # should make index=0 when the shape is one
...
class TensorData:
    ...
    def permute(self, *order: int) -> TensorData:
        ...
        st = []
        for o in order:
            # use (,) to add element into tuple
            sp.append(self.shape[o])
            st.append(self._strides[o])
        # use TensorData to construct a new Tensor data
        new_data = TensorData(self._storage, shape=tuple(sp), strides=tuple(st))
        # init requires storage,shape and strides
        return new_data"""
        return TensorData(
            self._storage,
