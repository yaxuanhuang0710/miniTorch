...
def tensor_zip(
    ...
    def _zip(
        ...
        if i < out_size:
            ...
            # write it to out
            out[o] = fn(a_storage[j], b_storage[m])
    ...
    if i < size:
        for j in [1, 2, 4, 8, 16]:
            ...
            if pos % (j * 2) == 0:
                ...
                cuda.syncthreads()
        # reduce to the last, copy cache to out
        if pos == 0:
            o = cuda.blockIdx.x
            out[o] = cache[0]
    ...
    for s in range(0, a_shape[-1], BLOCK_DIM):
        ...
        if s + pj < a_shape[-1] and i < a_shape[-2]:
            a_shared[pi, pj] = a_storage[
                batch * a_batch_stride + i * a_strides[1] + (s + pj) * a_strides[2]
            ]
        else:
            a_shared[pi, pj] = 0
        ...
        if s + pi < b_shape[-2] and j < b_shape[-1]:
            b_shared[pi, pj] = b_storage[
                batch * b_batch_stride + (s + pi) * b_strides[1] + j * b_strides[2]
            ]
        else:
            b_shared[pi, pj] = 0
        cuda.syncthreads()
